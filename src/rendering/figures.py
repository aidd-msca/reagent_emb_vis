...
from src.rendering.utils import to_cartesian_coordinates, uniform_sphere_points
...
def generate_sphere():
    ...
    return go.Surface(
        ...
        showscale=False,
        lighting=dict(
            ambient=1.0, diffuse=1.0, fresnel=0.0, specular=0.0, roughness=0.0
        ),  # Remove lighting
        opacity=1,
...
def update_sphere_chart(df: pd.DataFrame):
    ...
    return {"data": [scatter, sphere_surface], "layout": layout}
...
def update_class_selector(classes: dict[str, list]):
    ...
    colors = [
        px.colors.qualitative.Light24[i] if v else inactive_color
        for i, v in enumerate(active_classes_state)
    ]
    linewidth = [1 if v else 3 for i, v in enumerate(active_classes_state)]
    hovertext = [
        f"{class_names[i]} (on)" if v else f"{class_names[i]} (off)"
        for i, v in enumerate(active_classes_state)
    ]
    figure = dict(
        data=[
