    ...
    children=[
        html.Div(
            [
                ...
                dcc.Upload(id="csv-uploader", children=html.Button("Upload File")),
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        html.H6(
                            "Selector button: click on sections to hide/show reagent roles.",
                ...
                html.Div(
                    [
                        html.H6(
                            "Range slider: drag both ends to display only the points with indices in the selected range.",
                            style={"text-align": "center"},
                        ),
                ...
                html.Div(
                    [
                        dcc.Input(
                            value="",
                            id="filter-input",
                            placeholder="Filter: SMARTS pattern",
                            debounce=True,
                            style={"display": "none"},
                        )
                    ]
...
def display_hidden_elements(contents):
    ...
    return (
        style
        + range_slider_params
        + [update_flat_chart(data), update_class_selector(classes), classes]
    )
...
def update_graph(n_clicks, selected_class, slider_value, pattern_string, contents):
    ...
    data = data[
        data["numerical_label"].isin([i for i, v in enumerate(selected_class["active"]) if v])
    ]
...
def update_img_from_flat_map(hoverData):
    try:
        ...
        text_smiles = html.P(smiles, style={"font-size": "20px", "font-family": "Courier"})
        text_name = html.P(name, style={"font-size": "20px", "font-family": "Courier"})
    except:
        raise PreventUpdate
